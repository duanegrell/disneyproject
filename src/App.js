...
function App() {
  ...
  //FILTER THE OBJECT THAT WE ARE TARGETING FROM RIDESLIST
  function updatedArrayFromDelete(id){
    ...
    const filteredArray = ridesList.filter(ride => { 
      return ride.id !== id
    })
    setRidesList(filteredArray)
  }

  function handlePostRide(newRide) {
    setRidesList([...ridesList, newRide]);
  }
  ...
  const filteredScary = () => {
    if (scary === "") {
      ...
      return [...filteredRides()].filter(
        (ride) => ride.Ride_type_scary.includes(scary)
  ...
  const searchedRides = searchText
    ? ridesList.filter((ride) =>
        ride.Ride_name.toLowerCase().includes(searchText.toLowerCase())
      )
    : ridesList;
    ...
    <div className="App">
      <main>
        ...
        <Switch>
          ...
          <Route path="/newrideform">
            <NewRideForm handlePostRide={handlePostRide} />
          </Route>

          <Route path="/rides">
            <SearchBar setSearchText={setSearchText} />
            <Rides ridesList={searchedRides} searchText={searchText} onDelete={updatedArrayFromDelete}/>
          </Route>
